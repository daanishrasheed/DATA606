...
All three histograms can give an idea about the distribution of the
data, however, you can see the distribution much clearer when the bin
size is lower.
...
There are 68 flights that happen in February going to San Fransisco.
...
**The flight data shows that flights are generally landing early, but a
few heavy delays are dragging the average up. Since negative numbers
indicate early arrivals, the median of -11 minutes means that half of
all flights actually landed 11 or more minutes ahead of schedule.
However, the mean is higher at -4.5 minutes because it is heavily
influenced by a few severe delays. This is proven true from the
histogram that represents a few outliers, one having a 200 minute delay.
This histogram also reveals a right-skewed distribution, meaning that
while a typical traveler can realistically expect an early arrival, rare
and massive delays are heavily pulling the overall mathematical average
closer to zero.**
...
The selection of the month that has the lowest mean implies that you
have chosen the month which will protect you against the disaster. What
I really love about the mean measure is that it takes into account
months in which there occur some airport disasters such as thunderstorms
in summer and blizzards in winter. However, it should be noticed that
the presence of several extremely bad days will spoil the impression
from the whole month. If you decide to choose the month that has the
lowest median then you have chosen the month that has the best “typical”
day. The advantage is that you will get the information about the
experience of the average passenger excluding any accidents. The
disadvantage is that the median measure ignores delays that may occur.
...
**It would be best to fly out of LaGuardia Airport as it has the highest
percentage of being on time.**
...
Judging from the graph given, it seems that there is a exponentia
relationship between avg_speed and distance. As the distance grows, the
avg-speed taken by a plane increases, however, in most cases, it caps at
around 530 mph. It seems that no matter how much distance is in the
journey, the speed won't pass that cap (outside of one outlier).
...
```{r}
ggplot(data = filtered_flights, aes(x = dep_delay, y = arr_delay, color = carrier)) +  geom_point()
```
