...
**A streak length of 1 means he hit only one shot and followed it with a
miss. A streak length of 0 means the player missed following another
miss. Meaning that it is just 1 miss with zero hits beforehand.**
...
```{r plot-streak-kobe}
ggplot(data = kobe_streak_df, aes(x = unlist(kobe_streak))) +
  geom_bar()
```
...
The distribution of this chart is strongly right-skewed meaning that
most streaks were very short, so much so that the typical streak length
was 0. His longest streak was 4.
...
3.  In your simulation of flipping the unfair coin 100 times, how many
    ...
    Only 19 flips came up heads which makes sense as there was only 20%
    chance.
...
It is the same case as analyzing Kobe's streak in that most of the
independent shooter's streaks are 0. However, the longest streak is 8
whereas Kobe's was 4.
...
The second simulation will come out to be somewhat similar. Both follow
the same independent 45% shooting process, so they should share a
right-skewed shape and a typical streak of 0 or 1, but the exact counts
and longest streak will vary because the shots are randomly generated.
...
Kobe’s distribution of his streaks is almost identical to that of the
simulated independent shooter’s. The two distributions are both unimodal
and skewed to the right with the average streak length being 0. Since
Kobe’s streaks are not longer or more common than the independent
shooter’s streaks, it can be concluded that the hot hand does not fit
Kobe’s shooting pattern.
