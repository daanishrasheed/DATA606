...
knitr::opts_chunk$set(eval = TRUE, message = FALSE, warning = FALSE, output = TRUE)
library(tidyverse)
library(openintro)
```
...
years included in this data go from 1940 to 2002

```{r}
names(present)
```

Column names are "year", "boys", "girls"

```{r}
dim(present)
```

Dimensions of the data is 63 rows by 3 columns
...
1.  Make a plot that displays the proportion of boys born over time.
    What do you see?
    ...
    *Hint:* You should be able to reuse your code from Exercise 3 above, just replace the dataframe name.
    ...
    ![](images/clipboard-3639933949.png)

    After observing the plot above and comparing it with the arbuthnot, I can conclude that boys are born in greater proportion than girls even in the US.
    However, it is on a downward trend whereas in arbuthnot, there was no trend, as previously mentioned.
